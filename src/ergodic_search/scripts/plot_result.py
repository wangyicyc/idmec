...
def plot_hist(hist_data, save_path, title, baseline=None):
    ...
    n_groups = len(hist_data)  # 动态确定有多少组数据
    index = np.arange(len(hist_data[0]))  # 动态确定每组中有多少个值
    total_width = 0.8
    ...
    labels = ['baseline1','baseline2', 'my strategy']
    
    bars = []
    for i in range(n_groups):
        bar = ax.bar(positions[i], hist_data[i], bar_width,
    ...
    ax.set_xticks(index)
    ax.set_xticklabels([f'case {j+1}' for j in range(len(hist_data[0]))], fontsize=10)
...
datas = [[0.0 for _ in range(10)] for _ in range(3)]
# 定义要提取的类型顺序
