...
from multiRobots_lib.tools import find_first_connected_single, exchange_info_old, optimize_trajs, update_accumulated_time
...
# commandSaver = CommandToRosbag(bag_dir="../datas/bags/no_prob_connect")    
# pathSaver = PathToRosbag(bag_dir="../datas/bags/no_prob_connect")
decay_type = 'nodecay'
connection_threshold = opt_args["connect_threshold"]
connection_threshold = connection_threshold**2
map_merge_cnt = 0
init_dual = True
save_path = '../figures/ex1-baseline3.png'
...
while True:
    ...
    init_dual = False
    # ================================== 获取replan 信息 =========================================================
    current_time, robot_pair = find_first_connected_single(sol_trajs, connection_threshold, last_exchange_time)
    ...
    if map_merge_cnt >= map_merge_freq:
        ...
        for r_id in range(robot_number):
            ...
            # logging.info("filter distribution")
...
save_ergodic_metrics_to_yaml(global_metric, "baseline3", map_id=args.map_id)
