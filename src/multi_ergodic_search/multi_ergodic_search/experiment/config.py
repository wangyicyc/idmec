...
from experiment.settings import PACKAGE_ROOT, config, loaded

sys.path.insert(0, str(PACKAGE_ROOT))
from ergodic_planning.target_distribution import TargetDistribution # 构建目标分布
from ergodic_planning.dynamics import DoubleIntegrator, SingleIntegrator, HomoDynamics
from ergodic_planning.metric_utils import gaussian_function
...
def barrier_cost_upper(e):
    ...
    return jnp.maximum(jnp.exp(20 * (e - 1)), 1) - 1 - 1e-8
def barrier_cost_lower(e):
    """边界屏障函数，防止轨迹超出工作空间"""
    # return (jnp.maximum(0, e-1) + jnp.maximum(0, -e))
    return jnp.maximum(jnp.exp(-e * 20), 1) - 1 - 1e-8 

# some function
real_ws_bnds = jnp.array([
    [2.5 * config["avoidance_radius"], workspace_size - 2.5 * config["avoidance_radius"]], 
    [2.5 * config["avoidance_radius"], workspace_size - 2.5 * config["avoidance_radius"]]
    ])          # 工作空间边界,1x1米
